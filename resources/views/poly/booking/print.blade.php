        ...
        <div class="pageheader">
            ...
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a href="{{route('poly.booking.detail', ['id' => $master->id ])}}"> LPD PO NO: {{$master->lpd_po_no}}</a>
                    </li>
                    <li>
                        <a href="#"> Print View</a>
                    </li>
        ...
        <div class="add-nav">
            <div class="nav-heading">
                ...
                <span class="controls pull-right">
                    <a href="{{route('poly.booking.detail', ['id' => $master->id ])}}" class="btn btn-ef btn-ef-1 btn-ef-1-default btn-ef-1a btn-rounded-20 mr-5" data-toggle="tooltip" title="Back"><i class="fa fa-times"></i></a>
                                            ...
                                            <div class="table-responsive">
                                                <table class="table table-hover table-condensed" id="items">
                                                    ...
                                                    <tr style="height: 3px !important;">
                                                        ...
                                                        <th class="text-uppercase text-center" style=" font-size: xx-small !important;">Measurement</th>
                                                        <th class="text-uppercase text-center" style=" font-size: xx-small !important;">Order Quantity</th>
                                                        <th class="text-uppercase text-center" style=" font-size: xx-small !important;">Approved Unit Price</th>
                                                    ...
                                                    @foreach($details as $item)
                                                        @if($item->item_details == $uproduct->name)
                                                            <tr style="height: 1px !important;">
                                                                <td style="font-size: small !important;" class="text-center"><p>{{$item->measurement}} {{ (App\Helpers\Helper::IDwiseData('units','id',$item->width_length_unit_id))->short_unit }}</p></td>
                                                                <td style="font-size: small !important;" class="text-right"><P>{!! number_format($item->order_quantity, 0, '.', ',') !!} {{ (App\Helpers\Helper::IDwiseData('units','id',$item->order_unit_id))->short_unit }}</P></td>
                                                                @if(((App\Helpers\Helper::IDwiseData('poly_product_setups','id',$item->poly_product_setup_id))->poly_type) == 'GP')
                                                                    <td style="font-size: small !important;" class="text-right"><P>@if($item->currency == "৳") {{ $item->currency }} @else $ @endif {!! number_format($item->unit_price  , 13, '.', ',') !!} / {{$item->price_unit}}</P></td>
                                                                    <td style="font-size: small !important;" class="text-right"><P>$ {!! number_format($item->total_price/$item->order_quantity  , 11, '.', ',') !!} / {{ (App\Helpers\Helper::IDwiseData('units','id',$item->order_unit_id))->short_unit }}</P></td>
                                                                @elseif(((App\Helpers\Helper::IDwiseData('poly_product_setups','id',$item->poly_product_setup_id))->poly_type) == 'H&MG')
                                                                    <td style="font-size: small !important;" class="text-right"><P>@if($item->currency == "৳") {{ $item->currency }} @else $ @endif {!! number_format($item->unit_price  , 13, '.', ',') !!} / {{$item->price_unit}}</P></td>
                                                                    <td style="font-size: small !important;" class="text-right"><P>$ {!! number_format($item->total_price/$item->order_quantity  , 10, '.', ',') !!} / {{ (App\Helpers\Helper::IDwiseData('units','id',$item->order_unit_id))->short_unit }}</P></td>
                                                                @else
                                                                    <td style="font-size: small !important;" class="text-right"><P>@if($item->currency == "৳") {{ $item->currency }} @else $ @endif {!! number_format($item->unit_price  , 13, '.', ',') !!} / {{$item->price_unit}}</P></td>
                                                                    <td style="font-size: small !important;" class="text-right"><P>$ {!! number_format($item->total_price/$item->order_quantity  , 8, '.', ',') !!} / {{ (App\Helpers\Helper::IDwiseData('units','id',$item->order_unit_id))->short_unit }}</P></td>
                                                                @endif
                                                    ...
                                                    {{--<tfoot>
                                                    <tr style="height: 3px !important;">
                                                        <td colspan="3" style="font-size: xx-small !important;" class="text-right"><p><b>Total:</b></p></td>
